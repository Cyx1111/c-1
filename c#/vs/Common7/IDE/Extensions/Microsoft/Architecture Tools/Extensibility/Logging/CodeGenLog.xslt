...
<xsl:stylesheet version="1.0"
  ...
  <xsl:variable name="CodeGenFileName" select="/log:Log/log:Property/@CodeGenFileName" />
    ...
    <html>
      ...
      <body onLoad="displayOptions()">
        <h1>
          ...
          <a target="_blank">
            <xsl:attribute name="href"><xsl:value-of select="$CodeGenFileName" /></xsl:attribute>
            <xsl:value-of select="$CodeGenFileName" />
          </a>
        </h1>
        <p>
          <form class="Options">
            <input type="checkbox" id="Diagnostic" onClick="toggleDisplay('Diagnostic')" /><xsl:value-of select="key('itemClasses', 'Diagnostic')/@Option" /><br/>
            <input type="checkbox" id="Message" checked="true" onClick="toggleDisplay('Message')" /><xsl:value-of select="key('itemClasses', 'Message')/@Option" /><br/>
            <input type="checkbox" id="Warning" checked="true" onClick="toggleDisplay('Warning')" /><xsl:value-of select="key('itemClasses', 'Warning')/@Option" /><br/>
          </form>
        </p>
        <p>
          <table>
  ...
  <xsl:key name="itemClasses" match="log:MessageClass" use="@Name"/>
  <xsl:template match="*[@Text]" mode="items">
    ...
    <tr>
      ...
      <td><xsl:value-of select="key('itemClasses', name())/@Label"/></td>
      <td>:</td>
