...
//+VWD
window.selectCssRule = function (uid, ruleUid) {
...
//+VWD
        if (window.VWDHost) {
            window.VWDHost.NotifyAfterRefresh();
        }
//-VWD
    },
...
//+VWD
                    if (window.VWDHost) {
                        window.VWDHost.OpenStyleSheet(url);
                    }
                    else {
//-VWD
...
//+VWD
            if (window.VWDHost) {
                window.VWDHost.NotifyBeforeTreeViewSelection();
            }
//-VWD
...
//+VWD
            if (window.VWDHost) {
                window.VWDHost.NotifyAfterTreeViewSelection();

                if (notifyHost)
                    window.VWDHost.SelectionUpdateCompleted();                
            }
//-VWD
        });
    },
...
//+VWD
                if (window.VWDHost &&
                    !window.VWDHost.IsBrowserSelectionFrozen) {
                        callProxy("updateSelection",
                            [id,
                             function () {
                                 window.VWDHost.ExplorerElementSelected();
                             }]);
                    }
//-VWD
            };
...
//                        var nameColumn = "<span class='BPT-HTML-CSS-Selector' title='" + safeStyleSelector + "'> <span class='BPT-HTML-CSS-SelectorTag'>&lt;" + styleInGroup[0].inherited.toLowerCase() + "&gt; </span>" + styleSelector + "</span>";
                        var vwdSelectorAttributes = window.VWDHost ? " id=\"show" + (styleInGroup[0].inlined ? styleInGroup[0].uid : styleInGroup[0].ruleUID) + "\" style=\"cursor: pointer\" onclick=\"selectCssRule('" + uid + "'," + (styleInGroup[0].inlined ? "null" : ("'" + styleInGroup[0].ruleUID + "'")) + ")\"" : "";
...
//                        var nameColumn = "<input tabindex='-1' type='checkbox'" + (style.enabled ? "checked" : "") + "/>" + "<span class='BPT-HTML-CSS-Selector' title='" + safeStyleSelector + "'>" + styleName + "</span>";
                        var vwdSelectorAttributes = window.VWDHost ? " id=\"show" + (style.inlined ? style.uid : style.ruleUID) + "\" style=\"cursor: pointer\" onclick=\"selectCssRule('" + uid + "'," + (style.inlined ? "null" : ("'" + style.ruleUID + "'")) + ")\"" : "";
...
// SIG // MIIaZAYJKoZIhvcNAQcCoIIaVTCCGlECAQExCzAJBgUr
// SIG // DgMCGgUAMGcGCisGAQQBgjcCAQSgWTBXMDIGCisGAQQB
// SIG // gjcCAR4wJAIBAQQQEODJBs441BGiowAQS9NQkAIBAAIB
// SIG // AAIBAAIBAAIBADAhMAkGBSsOAwIaBQAEFNDA4ltxDLD4
// SIG // JA9sO9V2AXp5HYjkoIIVLTCCBKAwggOIoAMCAQICCmEZ
...
// SIG // zjEp/w7FW1zYTRuh2Povnj8uVRZryROj/TGCBKMwggSf
...
// SIG // MAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYE